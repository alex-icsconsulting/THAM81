...
1-0 Initial Issue 
1-1 15-09-2016 - ACWC, Infill values from 3 moved up a level and a new infill look up added for level three.
...
--#### Change made to this section 2016-09-15  - ACWC
--### Infill date changed from 4 to 5
ADMS_CONFIG.PKG_ADMS_UTIL.WRITE_TO_AUDIT_LOG(        NSCHEMA, V_PROCESSID,'RENOVATION DATE RULES STARTED','START',NULL , sysdate, null,ics_audit_id);
  ...
  SET (A.INFILL_VALUE_5, A.INFILL_VALUE_5_SRC) = 
(select 
   CASE 
...
-- DATEBUILT RULES
--#### Change made to this section 2016-09-15  - ACWC
--### Infill date changed from 5 to 6
...
(
  SELECT TO_DATE(T2.DATEBUILT)  INFILL_VALUE_6, 'TW DATEBUILT RULES' INFILL_VALUE_6_SRC --,T1.DATE_INSTALL
    FROM T_I_MAINS T1 
     LEFT JOIN WATERINF.T_I_MAINS_TWUL T2  ---THIS NEEDS TO COME FROM THE T_I_MAINS_TWUL TABLE
     ON T1.ICS_ASSET_ID = T2.ICS_ASSET_ID
...
-- YEARBUILTCODE RULES
--#### Change made to this section 2016-09-15  - ACWC
--###  Infill date changed from 6 to 7
...
(
  SELECT
    ...
    END INFILL_VALUE_7, 'TW YEARBUILTCODE RULES' INFILL_VALUE_7_SRC
   FROM T_I_MAINS T1 
     LEFT JOIN WATERINF.T_I_MAINS_TWUL T2  ---THIS NEEDS TO COME FROM THE T_I_MAINS_TWUL TABLE
     ON T1.ICS_ASSET_ID = T2.ICS_ASSET_ID
     WHERE T1.DATE_INSTALL IS NULL 
...
--#### Change made to this section 2016-09-15  - ACWC
--### Infill date changed from 7 to 8
UPDATE T_I_INFILL_DATE A SET (A.INFILL_VALUE_8, A.INFILL_VALUE_8_SRC) = 
(  SELECT  
    CASE 
    ...
    END INFILL_VALUE_8, 'TW MATERIAL RULES' INFILL_VALUE_8_SRC
   FROM T_I_MAINS T1 
    -- LEFT JOIN WATERINF.T_I_MAINS_TWUL T2  ---THIS NEEDS TO COME FROM THE T_I_MAINS_TWUL TABLE
    -- ON T1.ICS_ASSET_ID = T2.ICS_ASSET_ID
     WHERE T1.DATE_INSTALL IS NULL 
 ...
 WHERE A.INFILL_VALUE_8 IS NULL;
commit;
...
--Code entered -2016-06-15 by ACWC

--################ INFILL LEVEL THREE #####################
MERGE INTO 
  t_i_infill_date prod
USING
  (
  SELECT 
    A.ics_asset_id, 
    to_date('01-01-' || b.inferredyearbuilt, 'DD-MM-YYYY') "infill_value_3", 
    'INFERREDYEARBUILT' "infill_value_3_src" 
  FROM 
    (SELECT 
      genid, 
      gisid, 
      inferredyearbuilt
    FROM 
      waterinf_stage.t_r_mains 
    WHERE inferredyearbuilt IS NOT NULL AND  (lifecyclestatus = 'LIVE' OR (lifecyclestatus != 'LIVE' AND dateabandoned IS NULL))) b
  INNER JOIN
    (SELECT ics_asset_id, client_uid, asset_id FROM t_i_mains) A
  ON (to_char(A.client_uid) = to_char(b.gisid))
  ) stage
ON
  (prod.ics_id = stage.ics_asset_id)
WHEN MATCHED THEN UPDATE SET
  prod.infill_value_3 = stage.infill_value_3,
  prod.infill_value_3_src = prod.infill_value_3_src
WHEN NOT MATCHED THEN INSERT
  (prod.ics_id,prod.infill_value_3,prod.infill_value_3_src)
VALUES
  (
  stage.ics_asset_id, infill_value_3, infill_value_3_src
  );
...
--### Infill date changed from 3 to 4
...
USING
    ...
    END INFILL_VALUE_4, 'TW_RULES_CLEANUP02' INFILL_VALUE_4_SRC, ICS_ASSET_ID
     FROM T_I_MAINS
      
     ) DTINF
ON (A.ICS_ID = DTINF.ICS_ASSET_ID)
WHEN MATCHED THEN UPDATE SET
  A.INFILL_VALUE_4 = DTINF.INFILL_VALUE_4,
  A.INFILL_VALUE_4_SRC = DTINF.INFILL_VALUE_4_SRC
WHEN NOT MATCHED THEN INSERT
  (A.ICS_ID, A.INFILL_VALUE_4, A.INFILL_VALUE_4_SRC)
VALUES
  (DTINF.ICS_ASSET_ID,
  DTINF.INFILL_VALUE_4,
  DTINF.INFILL_VALUE_4_SRC 
  );
