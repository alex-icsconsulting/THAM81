...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "WATERINF"."VW_I_AIM_DISTRIBUTION_ADMS" as --("ICS_ASSET_ID", "COMMS_BURST_FIT_TUNED", "COMMS_LENGTH_CUST", "COMMS_LENGTH_TWUL", "DMA_AVG_COMMS", "DMA_AVG_COMMS_SIZE", "DMA_AVG_INSTALL", "DMA_DET_HRS_AVG", "DMA_ESPB_AVG", "DMA_LEAKAGE_AVG", "DMA_LEAKAGE_LOWEST", "MAINS_BURST_FIT_TUNED", "MAINS_COMMS_COUNT", "MAINS_COMMS_COUNT_L", "MAINS_COMMS_COUNT_M", "MAINS_COMMS_COUNT_S", "MAINS_CORR_FE", "MAINS_COST_ZONE", "MAINS_DG2", "MAINS_DIAM", "MAINS_DIAM_BIN", "MAINS_DMA", "MAINS_FAMILY_TYPE", "MAINS_INSTALL", "MAINS_ITS", "MAINS_ITS_MEAN", "MAINS_ITS_0_3", "MAINS_ITS_3_6", "MAINS_ITS_0_4", "MAINS_ITS_4_6", "MAINS_ITS_6_12", "MAINS_ITS_12_24", "MAINS_ITS_24_48", "MAINS_ITS_2_7", "MAINS_ITS_7", "MAINS_LENGTH", "MAINS_MATERIAL_BIN", "MAINS_MATERIAL_GROUP", "MAINS_MJR_ROAD", "MAINS_MNR_ROAD", "MAINS_MOTORWAY", "MAINS_RAIL", "MAINS_REGION", "MAINS_SALINE", "MAINS_SHRINK_SWELL", "MAINS_SURFACE_TYPE", "MAINS_WRZ", "AMBULANCESTATION", "AQUEDUCT", "BASEMENT", "COMMERCIAL", "EDUCATION", "FIRESTATION", "GOVERNMENT", "HOSPITAL", "INLANDWATER", "LISTEDBUILDING", "ELECTRICITYSUBSTATION", "TUNNELENTRANCE", "MOTORWAYAROAD", "SUBWAY", "POLICESTATION", "RAILWAYHIGHSPEED", "RAILWAY", "RAMSAR", "REDROUTE", "RESERVOIRSERVICEEMBANKMENT", "RESERVOIRSERVICEFLOOD", "RESERVOIRSTATUTORYEMBANKMENT", "RESERVOIRSTATUTORYFLOOD", "RESERVOIRSTORAGEEMBANKMENT", "RESERVOIRSTORAGEFLOOD", "RESIDENTIALPROPERTY", "SCHEDULEDMONUMENT", "SSSI", "UNDERGROUNDSTATION", "WORLDHERITAGESITE") AS 
  Select 
    core.ICS_ASSET_ID,
--BOROUGH_SCALAR   -not required
  CBC.COMMS_BURST_COUNT COMMS_BURST_COUNT,
  Comms_Model.COMMS_BURST_FIT_TUNED COMMS_BURST_FIT_TUNED,
 resleak.CSL_M * ASSET_LENGTH COMMS_CS_LEAKAGE_RES,
  resleak.PREDCOMMSCS * ASSET_LENGTH COMMS_LEAKAGE_CSL_FIT,
  resleak.PREDCOMMSTW * ASSET_LENGTH COMMS_LEAKAGE_TW_FIT,
  resleak.TWCOMMSL_M*ASSET_LENGTH COMMS_TW_LEAKAGE_RES
  Comms.COMMS_CS_LEN COMMS_LENGTH_CUST,
  Comms.COMMS_TW_LEN COMMS_LENGTH_TWUL,
  DMA_CC.DMA_COMMS_COUNT_S+DMA_CC.DMA_COMMS_COUNT_M+DMA_CC.DMA_COMMS_COUNT_L DMA_AVG_COMMS,
  DMA_CL.COMMS_CS_LEN+DMA_CL.COMMS_TW_LEN DMA_AVG_COMMS_SIZE,
  AGEDMA.avgage DMA_AVG_INSTALL,
  DMADETHRS.AVGDETHRS_4YRS DMA_DET_HRS_AVG,
  DMAESPB.AVG_ESPB DMA_ESPB_AVG,
  DMALEAK.AV4YRLEAK DMA_LEAKAGE_AVG,
  min_leak.LOW_LEAK_MEDIAN_16 DMA_LEAKAGE_LOWEST,
--DMA_METERING_FLAG  --not required
  bo.Borough MAINS_BOROUGHS,
  mbc.mains_burst_count MAINS_BURST_COUNT,
  ...
  costz.MAINS_COST_ZONE MAINS_COST_ZONE,
  case  
  ...
  resleak.preddistmains * ASSET_LENGTH MAINS_LEAKAGE_FIT,
  resleak.DISTL_M * ASSET_LENGTH MAINS_LEAKAGE_RES,
...
--MAINS_PIPE_DENSITY not this cycled
MP.MAINS_PRESSURE_AVG MAINS_PRESSURE,
  (Select NVL(ADMS_STRING_VALUE, 'BIN_99_UNKN') from ADMS_CONFIG.T_I_LKP_NUM where schema_name = 'WATERINF' and table_name = 'VW_I_AIM_DISTRIBUTION' and FIELD_NAME = 'PRESSURE_BIN' 
and NVL(MP.MAINS_PRESSURE_AVG,-5555) >= LOWER_VALUE
and NVL(MP.MAINS_PRESSURE_AVG,-5555) <= UPPER_VALUE) MAINS_PRESSURE_BIN,
  PCP.RESIDENTIAL+PCP.EXCLUDED+PCP.COMMERCIAL+PCP.FOOD+PCP.OTHER+PCP.CRITICAL) MAINS_PROPERTIES,
  (MPD.RESIDENTIAL_WEIGHTED_PROPCOUNT+
  MPD.COMMERCIAL_WEIGHTED_PROPCOUNT+
  MPD.FOOD_WEIGHTED_PROPCOUNT+
  MPD.OTHER_WEIGHTED_PROPCOUNT+
  MPD.TOTAL_WEIGHTED_PROPCOUNT) MAINS_PROP_DENSITY
  ...
  MWR.region_name MAINS_REGION_OPS,
  MNTW.RENOVATIONTYPE MAINS_RENOVATION,
--MAINS_ROAD_DETAIL
...
--SS_ID1
--SS_LENGTH
case when FLD_2D.ICS_ASSET_ID is not null then FLD_2D.UNDERGROUND_STATION else FLD_1D.UNDERGROUND_STATION end as UNDERGROUND, 
...
left join (Select ICS_ASSET_ID, cz.MAINS_COST_ZONE from T_P_MAINS_COST_ZONE mcz LEFT JOIN T_I_COST_ZONE cz on mcz.ICS_COST_ZONE_ID = cz.ICS_COST_ZONE_ID) costz on core.ICS_ASSET_ID = costz.ICS_ASSET_ID
left join VW_P_DMA_COMMS_COUNTS DMA_CC              on core.DMA =DMA_CC.DMA
left join VW_P_DMA_COMMS_LENGTHS DMA_CL             on core.DMA = DMA_CL.DMA
left join VW_P_DMA_AVG4YR_LEAKAGE DMALEAK           on core.DMA = DMALEAK.DMA
left join (Select DMA, avg((sysdate - DATE_INSTALL)/365.25) avgage from VW_I_ASSET_CORE group by DMA) AGEDMA on core.DMA = AGEDMA.DMA 
left join (Select ICS_ASSET_ID, Count(EVENT_TYPE) COMMS_BURST_COUNT from VW_P_COMMS_REPAIRS group by ICS_ASSET_ID) CBC on core.ICS_ASSET_ID = CBC.ICS_ASSET_ID 
left join  T_P_LEAKAGE_APP_PIPE  resleak on core.ICS_ASSET_ID = resleak.ICS_ASSET_ID
left join (SELECT ICS_ASSET_ID, Name Borough FROM T_P_BOROUGH_MAIN mn LEFT JOIN BASE.T_I_DISTRICT_BOROUGH bo on mn.ICS_DISTRICT_BOROUGH_ID = bo.ICS_DISTRICT_BOROUGH_ID) bo on core.ICS_ASSET_ID = bo.ICS_ASSET_ID
left join (Select ICS_ASSET_ID, count(ICS_ASSET_EVENT_ID) mains_burst_count from T_I_ASSET_EVENTS group by ICS_ASSET_ID) MBC on core.ICS_ASSET_ID = MBC.ICS_ASSET_ID
left join VW_P_MAINS_PRESSURE MP on core.ICS_ASSET_ID = MP.ICS_ASSET_ID
left join T_P_MAINSPROPERTYDENSITY MPD on core.ICS_ASSET_ID = MPD.ICS_ASSET_ID
left join T_P_PROPERTYCOUNTPERMAINS PCP on core.ICS_ASSET_ID on PCP.ICS_ASSET_ID
left join VW_P_MAINS_WORK_REGION MWR on core.ICS_ASSET_ID = MWR.ICS_ASSET_ID
